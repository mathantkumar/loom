    ...
    if (isUser) {
        return (
            <div className="flex justify-end mb-6 animate-in slide-in-from-bottom-2 fade-in duration-300">
                <div className="bg-gray-900 text-white px-5 py-3 rounded-2xl rounded-tr-sm max-w-[80%] shadow-md text-sm font-medium leading-relaxed">
                    {/* Render user query as single evidence/action key if needed, but usually just text content stored in a section? 
                        Wait, we store sections. For user, let's assume we store the text in 'root_cause' or a special 'query' key 
                        or just pass raw text props. 
                        Let's adjust type to have optional 'text' for user role.
                    */}
                    {message.sections['query']?.content || "..."}
                </div>
                <div className="w-8 h-8 rounded-full bg-gray-200 ml-3 flex-shrink-0 flex items-center justify-center">
                    ...
                    {SECTION_KEYS.map(key => {
                        ...
                        return (
                            <div key={key} className="animate-in fade-in slide-in-from-bottom-1 duration-500">
                                ...
                                {key === 'root_cause' && (
                                    <div className="bg-white border border-gray-200 rounded-lg p-3 text-sm text-gray-800 leading-relaxed shadow-sm">
                                        {section.content}
                                    </div>
                                )}
...
export const AskSentinelPage: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-[#FAFAFA] relative min-h-0 font-sans selection:bg-purple-100">
            {/* Header */}
            <header className="sticky top-0 left-0 right-0 h-14 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-6 z-30 flex-shrink-0">
